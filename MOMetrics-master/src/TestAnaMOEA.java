...
//        prepareFronts(minimizing, new String[]{pathSS+"/"+instanceName, pathPrev+"/"+instanceName}, pathOutRef);
    ...
    public static void evaluateDir(String[] args) throws FileNotFoundException {
        String pathGlobal = (args.length > 0) ? args[0] : "/Users/jesus.sanchezoro/Downloads/ana_problem/executor/pareto";
        String[] paths = new File(pathGlobal).list((dir, name) -> dir.isDirectory() && !name.startsWith(".") && !name.startsWith("jmetal") && !name.contains("csv") && !name.contains("xlsx"));
...
//        };
        ...
        try (PrintWriter pw = new PrintWriter("anamoea_analysis.csv")) {
            String matching = "";
